...
from peft import get_peft_config, get_peft_model, LoraConfig, TaskType, set_peft_model_state_dict
...
def main(train: Path, val: Path, model_id: str, ckpt: Optional[Path], spk_emb_ckpt: Optional[Path]):
    ...
    # to handle this
    eval_val_data = DataLoader(val_dataset, batch_size, shuffle=False)
    ...
    # for now.
    eval_train_data = DataLoader(train_dataset, batch_size, shuffle=False)
        ...
        if master_process:
            if iter_num % eval_interval == 0 and master_process:
                ...
                if save_checkpoint and iter_num > 0:
                    ...
                    # torch.save(lora_state, os.path.join(ckpts_save_dir, ckpt_save_name))
